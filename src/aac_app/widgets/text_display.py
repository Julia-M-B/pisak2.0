...
class PisakDisplay(QLabel, EventEmitter):
    """
    Text display. Manages displaying and modifying text.
    It has its own cursor indicating where the text is being modified.
    It ignores input from the physical keyboard - it only accepts events from the virtual keyboard.
    """
    ...
    def _calculate_max_lines(self) -> int:
        ...
        return max(
            2, available_height // line_height - 1
        )  # subtract one line for the "page number"
    ...
    def _calculate_max_line_length(self) -> int:
        ...
        horizontal_padding = 14 + int(
            avg_char_width * 1.5
        )  # Extra 1.5 chars worth of space
    ...
    def init_ui(self):
        ...
        )  # Align text to top-left for better multiline support
    ...
    def move_cursor_down(self):
        ...
        if cursor_line_idx < (len(lines) - 1) and len(lines) > 1:
            ...
            if len(next_line) <= line_length:
                self._cursor_index += (
                    len(cursor_line) + 1 - line_length + len(next_line)
                )  # if next line is shorter, move at the end of the next line
            else:
    ...
    def _wrap_text(self, text, max_width):
        ...
        The goal is to prevent words from being split when they do not fit
        on a line (in that case the whole last word is moved to the next line).
        """
        if not text:
            return [""], 0
        ...
        for p_idx, paragraph in enumerate(paragraphs):
            ...
            for word_idx, word in enumerate(words):
                ...
                if (
                    current_width
                    + (space_width if needs_space_before else 0)
                    + word_width
                    <= max_width
                ):
                    # Word fits
    ...
    def update_display(self):
        ...
        margin_top = int(
            line_height * 0.2
        )  # A small push down, relying on the empty line buffer
