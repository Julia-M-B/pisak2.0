...
class PisakStackedWidget(QStackedWidget, PisakScannableItem):
    """
    Stores different objects and manages their display:
    ...
    def switch_shown_item(self, new_item):
        """
        Change the currently displayed widget.
        After the displayed widget changes, an "ITEMS_SWITCHED" signal is emitted
        carrying information about which widget is now displayed (so that its
        scanning can be started).
